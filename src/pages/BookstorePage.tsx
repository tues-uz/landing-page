...
import { BOOKSTORE_DEFAULTS } from "@/locales/bookstoreDefaults";

function trBookstore(t: TFunction, key: keyof typeof BOOKSTORE_DEFAULTS) {
  return t(key, { defaultValue: BOOKSTORE_DEFAULTS[key] });
}
...
export default function BookstorePage() {
  ...
  const navLeafLabel = th("secondNavStudentLife.bookstore");
...
function BookstoreArticle({ t }: { t: TFunction }) {
  const title = trBookstore(t, "bookstorePageTitle");
  const bodyRaw = trBookstore(t, "bookstorePageBody");
  const heroSrc = trBookstore(t, "bookstoreHeroSrc");
  const heroAlt = trBookstore(t, "bookstoreHeroAlt");
