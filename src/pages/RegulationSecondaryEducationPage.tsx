...
import {
  REGULATION_SECONDARY_EDUCATION_HERO_SRC,
  REGULATION_SECONDARY_EDUCATION_PAGE_DEFAULTS,
} from "@/locales/regulationSecondaryEducationDefaults";
import { cn } from "@/lib/utils";

function trReg(t: TFunction, key: keyof typeof REGULATION_SECONDARY_EDUCATION_PAGE_DEFAULTS) {
  return t(key, { defaultValue: REGULATION_SECONDARY_EDUCATION_PAGE_DEFAULTS[key] });
}

export default function RegulationSecondaryEducationPage() {
  const { t } = useTranslation("topNav");
  const { t: tCommon } = useTranslation("common");
  const { t: th } = useTranslation("header");

  const admissionLabel = th("secondNav.admission2025");
  const leafTitle = trReg(t, "regulationSecondaryEducationPageTitle");
...
function RegulationArticle({ t }: { t: TFunction }) {
  const title = trReg(t, "regulationSecondaryEducationPageTitle");
  const alt = trReg(t, "regulationSecondaryEducationHeroAlt");
  const bodyRaw = trReg(t, "regulationSecondaryEducationBody");
  ...
  return (
    <article className="max-w-none">
      <figure className="relative aspect-[16/10] w-full overflow-hidden rounded-xl border border-border bg-muted shadow-sm sm:aspect-[16/9]">
        <img
          src={REGULATION_SECONDARY_EDUCATION_HERO_SRC}
          alt={alt}
...
function renderBlock(p: string, i: number) {
