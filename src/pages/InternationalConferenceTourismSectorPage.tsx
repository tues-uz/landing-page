...
export default function InternationalConferenceTourismSectorPage() {
  ...
  const leafLabel = trConf(t, "internationalConferencesCard2BreadcrumbLabel");
...
function ConferenceArticle({ t }: { t: TFunction }) {
  const title = trConf(t, "internationalConferencesCard2PageTitle");
  const alt = trConf(t, "internationalConferencesCard2HeroAlt");
  const bodyRaw = trConf(t, "internationalConferencesCard2DetailBody");
  ...
  return (
    <article className="max-w-none">
      <figure className="relative aspect-[16/10] w-full overflow-hidden rounded-xl border border-border bg-muted shadow-sm sm:aspect-[16/9]">
        <img
          src={INTERNATIONAL_CONFERENCE_CARD2_HERO_SRC}
          alt={alt}
