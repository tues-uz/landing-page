...
export default function AdvancedTrainingForeignTeachersGuangzhouPage() {
  ...
  const leafLabel = trAtt(t, "advancedTrainingForeignTeachersCard3BreadcrumbLabel");
...
function GuangzhouArticle({ t }: { t: TFunction }) {
  const title = trAtt(t, "advancedTrainingForeignTeachersCard3PageTitle");
  const alt = trAtt(t, "advancedTrainingForeignTeachersCard3HeroAlt");
  const bodyRaw = trAtt(t, "advancedTrainingForeignTeachersCard3DetailBody");
  ...
  return (
    <article className="max-w-none">
      <figure className="relative aspect-[16/10] w-full overflow-hidden rounded-xl border border-border bg-muted shadow-sm sm:aspect-[16/9]">
        <img
          src={ADVANCED_TRAINING_FOREIGN_TEACHERS_CARD3_HERO_SRC}
          alt={alt}
