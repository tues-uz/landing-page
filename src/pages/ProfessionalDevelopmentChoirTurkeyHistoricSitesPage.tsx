...
export default function ProfessionalDevelopmentChoirTurkeyHistoricSitesPage() {
  ...
  const leafLabel = trChoir(t, "professionalDevelopmentChoirCard5BreadcrumbLabel");
...
function ChoirDetailArticle({ t }: { t: TFunction }) {
  const title = trChoir(t, "professionalDevelopmentChoirCard5PageTitle");
  const alt = trChoir(t, "professionalDevelopmentChoirCard5HeroAlt");
  const bodyRaw = trChoir(t, "professionalDevelopmentChoirCard5DetailBody");
  ...
  return (
    <article className="max-w-none">
      <figure className="relative aspect-[16/10] w-full overflow-hidden rounded-xl border border-border bg-muted shadow-sm sm:aspect-[16/9]">
        <img
          src={PROFESSIONAL_DEVELOPMENT_CHOIR_CARD5_HERO_SRC}
          alt={alt}
