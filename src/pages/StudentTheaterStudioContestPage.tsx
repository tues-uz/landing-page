...
import { STUDENT_THEATER_STUDIO_CONTEST_DEFAULTS } from "@/locales/studentTheaterStudioContestDefaults";

function trTheaterContest(t: TFunction, key: keyof typeof STUDENT_THEATER_STUDIO_CONTEST_DEFAULTS) {
  return t(key, { defaultValue: STUDENT_THEATER_STUDIO_CONTEST_DEFAULTS[key] });
}
...
export default function StudentTheaterStudioContestPage() {
  const { t } = useTranslation("topNav");
  const { t: tCommon } = useTranslation("common");
  const { t: th } = useTranslation("header");

  const leafTitle = trTheaterContest(t, "studentTheaterContestPageTitle");
        ...
        <div className="container mx-auto max-w-[1348px] px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-12 lg:gap-8">
            <div className="order-1 lg:order-none lg:col-span-9">
              <StudentTheaterStudioContestArticle t={t} />
            </div>
...
function StudentTheaterStudioContestArticle({ t }: { t: TFunction }) {
  const title = trTheaterContest(t, "studentTheaterContestPageTitle");
  const bodyRaw = trTheaterContest(t, "studentTheaterContestPageBody");
  const heroSrc = trTheaterContest(t, "studentTheaterContestHeroSrc");
