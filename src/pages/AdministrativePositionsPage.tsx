...
import { ADMINISTRATIVE_POSITIONS_PAGE_DEFAULTS } from "@/locales/administrativePositionsDefaults";

function trAdministrativePositions(
  t: TFunction,
  key: keyof typeof ADMINISTRATIVE_POSITIONS_PAGE_DEFAULTS,
) {
  return t(key, { defaultValue: ADMINISTRATIVE_POSITIONS_PAGE_DEFAULTS[key] });
}

export default function AdministrativePositionsPage() {
  const { t } = useTranslation("topNav");
  const { t: tCommon } = useTranslation("common");
  const { t: th } = useTranslation("header");

  const pageTitle = trAdministrativePositions(t, "administrativePositionsPageTitle");
      ...
      <main className="below-header">
        ...
        <div className="container mx-auto flex min-h-[60vh] max-w-[1348px] items-center px-4 py-16 sm:px-6 sm:py-20 lg:px-8 lg:py-24">
          <article className="mx-auto flex max-w-xl flex-col items-center text-center">
            <img
              src={trAdministrativePositions(t, "administrativePositionsEmptyImageSrc")}
              alt={trAdministrativePositions(t, "administrativePositionsEmptyImageAlt")}
            ...
            <p className="mt-3 text-body-article text-muted-foreground">
              {trAdministrativePositions(t, "administrativePositionsEmptyMessage")}
            </p>
          </article>
        </div>
      </main>
