...
import { FINE_APPLIED_ARTS_CLUB_DEFAULTS } from "@/locales/fineAppliedArtsClubDefaults";

function trFineArts(t: TFunction, key: keyof typeof FINE_APPLIED_ARTS_CLUB_DEFAULTS) {
  return t(key, { defaultValue: FINE_APPLIED_ARTS_CLUB_DEFAULTS[key] });
}
...
export default function FineAppliedArtsClubPage() {
  const { t } = useTranslation("topNav");
  const { t: tCommon } = useTranslation("common");
  const { t: th } = useTranslation("header");

  const leafTitle = trFineArts(t, "fineAppliedArtsClubPageTitle");
        ...
        <div className="container mx-auto max-w-[1348px] px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-12 lg:gap-8">
            <div className="order-1 lg:order-none lg:col-span-9">
              <FineAppliedArtsClubArticle t={t} />
            </div>
...
function FineAppliedArtsClubArticle({ t }: { t: TFunction }) {
  const title = trFineArts(t, "fineAppliedArtsClubPageTitle");
  const bodyRaw = trFineArts(t, "fineAppliedArtsClubPageBody");
  const heroSrc = trFineArts(t, "fineAppliedArtsClubHeroSrc");
