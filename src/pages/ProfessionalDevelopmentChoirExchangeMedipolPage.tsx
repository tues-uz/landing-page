...
export default function ProfessionalDevelopmentChoirExchangeMedipolPage() {
  ...
  const leafLabel = trChoir(t, "professionalDevelopmentChoirCard1BreadcrumbLabel");
...
function ChoirDetailArticle({ t }: { t: TFunction }) {
  const title = trChoir(t, "professionalDevelopmentChoirCard1PageTitle");
  const alt = trChoir(t, "professionalDevelopmentChoirCard1HeroAlt");
  const bodyRaw = trChoir(t, "professionalDevelopmentChoirCard1DetailBody");
  ...
  return (
    <article className="max-w-none">
      <figure className="relative aspect-[16/10] w-full overflow-hidden rounded-xl border border-border bg-muted shadow-sm sm:aspect-[16/9]">
        <img
          src={PROFESSIONAL_DEVELOPMENT_CHOIR_CARD1_HERO_SRC}
          alt={alt}
