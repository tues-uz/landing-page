...
} from "@/locales/studentAcademicSupportDefaults";
import { cn } from "@/lib/utils";

function trSupport(t: TFunction, key: keyof typeof STUDENT_ACADEMIC_SUPPORT_PAGE_DEFAULTS) {
  return t(key, { defaultValue: STUDENT_ACADEMIC_SUPPORT_PAGE_DEFAULTS[key] });
}
...
export default function StudentAcademicSupportPage() {
  const { t } = useTranslation("topNav");
  const { t: tCommon } = useTranslation("common");
  const { t: th } = useTranslation("header");

  const pageTitle = th("secondNavStudentLife.studentAcademicSupport");
        ...
        <div className="container mx-auto max-w-[1348px] px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-12 lg:gap-8">
            <div className="order-1 lg:order-none lg:col-span-9">
              <StudentAcademicSupportContent t={t} pageTitle={pageTitle} />
            </div>
...
function StudentAcademicSupportContent({ t, pageTitle }: { t: TFunction; pageTitle: string }) {
  const intro = trSupport(t, "studentAcademicSupportPageIntro");
  ...
  return (
    <div className="mt-0 max-w-none">
      ...
      <ul className="mt-10 grid grid-cols-1 gap-[16px] md:grid-cols-3" role="list">
        {STUDENT_ACADEMIC_SUPPORT_CARDS.map((card, index) => {
          const inner = (
            <>
          ...
          return (
            <li key={card.title} className="flex min-h-0">
              {card.to ? (
                <Link to={card.to} className={linkedClass} aria-label={card.title}>
                  {inner}
                </Link>
              ) : (
