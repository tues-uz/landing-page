...
} from "@/locales/communityClubsDefaults";
import { cn } from "@/lib/utils";

function trClubs(t: TFunction, key: keyof typeof COMMUNITY_CLUBS_PAGE_DEFAULTS) {
  return t(key, { defaultValue: COMMUNITY_CLUBS_PAGE_DEFAULTS[key] });
}
...
export default function CommunityClubsPage() {
  ...
  const pageTitle = th("secondNavStudentLife.communityClubs");
        ...
        <div className="container mx-auto max-w-[1348px] px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-12 lg:gap-8">
            <div className="order-1 lg:order-none lg:col-span-9">
              <CommunityClubsContent t={t} pageTitle={pageTitle} />
            </div>
...
function CommunityClubsContent({ t, pageTitle }: { t: TFunction; pageTitle: string }) {
  const intro = trClubs(t, "communityClubsPageIntro");
  ...
  return (
    <div className="mt-0 max-w-none">
      ...
      <ul
        ...
        role="list"
      >
        {COMMUNITY_CLUB_CARDS.map((card, index) => {
          const inner = (
            <>
          ...
          return (
            <li key={index} className="flex min-h-0">
              {card.to ? (
