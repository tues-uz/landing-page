...
import { CAFETERIAS_DEFAULTS } from "@/locales/cafeteriasDefaults";

function trCafeterias(t: TFunction, key: keyof typeof CAFETERIAS_DEFAULTS) {
  return t(key, { defaultValue: CAFETERIAS_DEFAULTS[key] });
}
...
export default function CafeteriasPage() {
  ...
  const navLeafLabel = th("secondNavStudentLife.cafeterias");
...
function CafeteriasArticle({ t }: { t: TFunction }) {
  const title = trCafeterias(t, "cafeteriasPageTitle");
  const bodyRaw = trCafeterias(t, "cafeteriasPageBody");
  const heroSrc = trCafeterias(t, "cafeteriasHeroSrc");
