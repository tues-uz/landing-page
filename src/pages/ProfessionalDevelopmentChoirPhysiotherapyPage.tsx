...
export default function ProfessionalDevelopmentChoirPhysiotherapyPage() {
  ...
  const leafLabel = trChoir(t, "professionalDevelopmentChoirCard4BreadcrumbLabel");
...
function ChoirDetailArticle({ t }: { t: TFunction }) {
  const title = trChoir(t, "professionalDevelopmentChoirCard4PageTitle");
  const alt = trChoir(t, "professionalDevelopmentChoirCard4HeroAlt");
  const bodyRaw = trChoir(t, "professionalDevelopmentChoirCard4DetailBody");
  ...
  return (
    <article className="max-w-none">
      <figure className="relative aspect-[16/10] w-full overflow-hidden rounded-xl border border-border bg-muted shadow-sm sm:aspect-[16/9]">
        <img
          src={PROFESSIONAL_DEVELOPMENT_CHOIR_CARD4_HERO_SRC}
          alt={alt}
