...
const HERO_SRC = "/images/internationalization/grant-japan-matsumae.png";

export default function InternationalGrantJapanMatsumaePage() {
  ...
  const leafLabel = trGrants(t, "internationalGrantJapanBreadcrumbLabel");
...
function JapanMatsumaeArticle({ t }: { t: TFunction }) {
  const title = trGrants(t, "internationalGrantJapanPageTitle");
  const alt = trGrants(t, "internationalGrantJapanHeroAlt");
  const p1 = trGrants(t, "internationalGrantJapanParagraph1");
  const p2 = trGrants(t, "internationalGrantJapanParagraph2");
  const benefitsHeading = trGrants(t, "internationalGrantJapanBenefitsHeading");
  const benefitsRaw = trGrants(t, "internationalGrantJapanBenefitsItems");
  const benefits = parseEnDashItems(benefitsRaw);
  const applicationPeriod = trGrants(t, "internationalGrantJapanApplicationPeriod");
  ...
  return (
    <article className="max-w-none">
      ...
      <p className="mt-6 text-body-article text-muted-foreground">{p1}</p>
      <p className="mt-4 text-body-article text-muted-foreground">{p2}</p>
      ...
      <p className="mt-10 text-body-article font-medium text-foreground">{applicationPeriod}</p>
    </article>
  );
}
