...
export default function AdvancedTrainingForeignTeachersFzuPage() {
  ...
  const leafLabel = trAtt(t, "advancedTrainingForeignTeachersCard1BreadcrumbLabel");
...
function FzuArticle({ t }: { t: TFunction }) {
  const title = trAtt(t, "advancedTrainingForeignTeachersCard1PageTitle");
  const alt = trAtt(t, "advancedTrainingForeignTeachersCard1HeroAlt");
  const bodyRaw = trAtt(t, "advancedTrainingForeignTeachersCard1DetailBody");
  ...
  return (
    <article className="max-w-none">
      <figure className="relative aspect-[16/10] w-full overflow-hidden rounded-xl border border-border bg-muted shadow-sm sm:aspect-[16/9]">
        <img
          src={ADVANCED_TRAINING_FOREIGN_TEACHERS_CARD1_HERO_SRC}
          alt={alt}
