...
export default function ProfessionalDevelopmentChoirMedipolClinicPage() {
  ...
  const leafLabel = trChoir(t, "professionalDevelopmentChoirCard2BreadcrumbLabel");
...
function ChoirDetailArticle({ t }: { t: TFunction }) {
  const title = trChoir(t, "professionalDevelopmentChoirCard2PageTitle");
  const alt = trChoir(t, "professionalDevelopmentChoirCard2HeroAlt");
  const bodyRaw = trChoir(t, "professionalDevelopmentChoirCard2DetailBody");
  ...
  return (
    <article className="max-w-none">
      <figure className="relative aspect-[16/10] w-full overflow-hidden rounded-xl border border-border bg-muted shadow-sm sm:aspect-[16/9]">
        <img
          src={PROFESSIONAL_DEVELOPMENT_CHOIR_CARD2_HERO_SRC}
          alt={alt}
