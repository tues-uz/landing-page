...
export default function InternationalConferenceLinguisticsLanguageEducationPage() {
  ...
  const leafLabel = trConf(t, "internationalConferencesCard4BreadcrumbLabel");
...
function ConferenceArticle({ t }: { t: TFunction }) {
  const title = trConf(t, "internationalConferencesCard4PageTitle");
  const alt = trConf(t, "internationalConferencesCard4HeroAlt");
  const bodyRaw = trConf(t, "internationalConferencesCard4DetailBody");
  ...
  return (
    <article className="max-w-none">
      <figure className="relative aspect-[16/10] w-full overflow-hidden rounded-xl border border-border bg-muted shadow-sm sm:aspect-[16/9]">
        <img
          src={INTERNATIONAL_CONFERENCE_CARD4_HERO_SRC}
          alt={alt}
