...
export default function InternationalConferencePreventiveMedicinePage() {
  ...
  const leafLabel = trConf(t, "internationalConferencesCard3BreadcrumbLabel");
...
function ConferenceArticle({ t }: { t: TFunction }) {
  const title = trConf(t, "internationalConferencesCard3PageTitle");
  const alt = trConf(t, "internationalConferencesCard3HeroAlt");
  const bodyRaw = trConf(t, "internationalConferencesCard3DetailBody");
  ...
  return (
    <article className="max-w-none">
      <figure className="relative aspect-[16/10] w-full overflow-hidden rounded-xl border border-border bg-muted shadow-sm sm:aspect-[16/9]">
        <img
          src={INTERNATIONAL_CONFERENCE_CARD3_HERO_SRC}
          alt={alt}
