...
import { SCIENCE_CERTIFICATE_CARDS, getScienceCertificateById } from "@/data/scienceCertificateCards";
...
const HUB_PATH = "/science/certificates";

export default function ScienceCertificateDetailPage() {
  const { certificateId } = useParams<{ certificateId: string }>();
  const { t } = useTranslation("header");
  const { t: tCommon } = useTranslation("common");

  const card = getScienceCertificateById(certificateId);
  if (!card) {
    return <Navigate to={HUB_PATH} replace />;
  }

  const hubTitle = t("scienceCertificatesPageTitle");
  const title = t(card.titleKey);
  const paragraphs = getCardBodyParagraphs(t, card.bodyKey);
  const imgIndex = SCIENCE_CERTIFICATE_CARDS.findIndex((c) => c.id === card.id) + 1;
