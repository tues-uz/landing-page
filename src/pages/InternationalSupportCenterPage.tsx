...
import { Globe, Home, LifeBuoy } from "lucide-react";
...
const CARD_ICONS: LucideIcon[] = [LifeBuoy];
...
export default function InternationalSupportCenterPage() {
  const { t } = useTranslation("topNav");
  const { t: tCommon } = useTranslation("common");
  const { t: th } = useTranslation("header");

  const leafTitle = trIsc(t, "internationalSupportCenterPageTitle");
        ...
        <div className="container mx-auto max-w-[1348px] px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-12 lg:gap-8">
            <div className="order-1 lg:order-none lg:col-span-9">
              <InternationalSupportCenterArticle t={t} />
            </div>
...
function InternationalSupportCenterArticle({ t }: { t: TFunction }) {
  const title = trIsc(t, "internationalSupportCenterPageTitle");
  const introRaw = trIsc(t, "internationalSupportCenterPageIntro");
  ...
  return (
    <article className="max-w-none">
      ...
      <ul className="mt-10 grid grid-cols-1 gap-[16px] md:grid-cols-3" role="list">
        {INTERNATIONAL_SUPPORT_CENTER_CARD_KEYS.map((cardKey, index) => {
          const Icon = CARD_ICONS[index] ?? Globe;
          const cardTitle = trIsc(t, cardKey);
          const href = INTERNATIONAL_SUPPORT_CENTER_CARD_HREFS[index];
          const previewSrc = INTERNATIONAL_SUPPORT_CENTER_CARD_IMAGES[index];
