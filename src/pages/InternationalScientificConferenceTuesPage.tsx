...
export default function InternationalScientificConferenceTuesPage() {
  ...
  const leafLabel = trConf(t, "internationalConferencesCard1BreadcrumbLabel");
...
function ConferenceArticle({ t }: { t: TFunction }) {
  const title = trConf(t, "internationalConferencesCard1PageTitle");
  const alt = trConf(t, "internationalConferencesCard1HeroAlt");
  const bodyRaw = trConf(t, "internationalConferencesCard1DetailBody");
  ...
  return (
    <article className="max-w-none">
      <figure className="relative aspect-[16/10] w-full overflow-hidden rounded-xl border border-border bg-muted shadow-sm sm:aspect-[16/9]">
        <img
          src={INTERNATIONAL_CONFERENCE_CARD1_HERO_SRC}
          alt={alt}
