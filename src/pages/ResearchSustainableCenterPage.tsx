...
import { RESEARCH_SUSTAINABLE_CENTER_CARDS } from "@/data/researchSustainableCenterCards";
import { bachelorFullTimeCardImageSrc } from "@/data/bachelorFullTimeCardImages";

export default function ResearchSustainableCenterPage() {
  const { t } = useTranslation("header");
  const { t: tCommon } = useTranslation("common");
  const title = t("centerResearchSustainablePageTitle", {
    defaultValue: "Centers for research and sustainable innovation",
  });
  const intro = t("centerResearchSustainablePageIntro", {
    defaultValue:
      "TUES units that coordinate research activity, sustainable development priorities, and innovation support across the university. The list is updated as new structures are established.",
  });
            ...
            <div className="order-1 lg:order-none lg:col-span-9">
              <article className="max-w-none">
                ...
                <ul
                  ...
                  role="list"
                >
                  {RESEARCH_SUSTAINABLE_CENTER_CARDS.map((item, i) => {
                    const imgSrc = bachelorFullTimeCardImageSrc(i + 1);
                    return (
                      <li key={item.id} className="flex min-w-0 flex-col">
                        <Link
                          to={`/science/center-research-sustainable-innovation/${item.id}`}
