...
export default function InternationalConferenceLinguisticsLiteraryStudiesPage() {
  ...
  const leafLabel = trConf(t, "internationalConferencesCard6BreadcrumbLabel");
...
function ConferenceArticle({ t }: { t: TFunction }) {
  const title = trConf(t, "internationalConferencesCard6PageTitle");
  const alt = trConf(t, "internationalConferencesCard6HeroAlt");
  const bodyRaw = trConf(t, "internationalConferencesCard6DetailBody");
  ...
  return (
    <article className="max-w-none">
      <figure className="relative aspect-[16/10] w-full overflow-hidden rounded-xl border border-border bg-muted shadow-sm sm:aspect-[16/9]">
        <img
          src={INTERNATIONAL_CONFERENCE_CARD6_HERO_SRC}
          alt={alt}
