...
import { GraduationCap, Globe, Home, Megaphone } from "lucide-react";
...
const CARD_ICONS: LucideIcon[] = [Globe, GraduationCap, Megaphone];

export default function InternationalGrantsPage() {
  const { t } = useTranslation("topNav");
  const { t: tCommon } = useTranslation("common");
  const { t: th } = useTranslation("header");

  const leafTitle = trGrants(t, "internationalGrantsPageTitle");
        ...
        <div className="container mx-auto max-w-[1348px] px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-12 lg:gap-8">
            <div className="order-1 lg:order-none lg:col-span-9">
              <InternationalGrantsArticle t={t} />
            </div>
...
function InternationalGrantsArticle({ t }: { t: TFunction }) {
  const title = trGrants(t, "internationalGrantsPageTitle");
  const introRaw = trGrants(t, "internationalGrantsPageIntro");
  ...
  return (
    <article className="max-w-none">
      ...
      <ul className="mt-10 grid grid-cols-1 gap-[16px] md:grid-cols-3" role="list">
        {INTERNATIONAL_GRANTS_CARD_KEYS.map((cardKey, index) => {
          const Icon = CARD_ICONS[index] ?? Globe;
          const cardTitle = trGrants(t, cardKey);
          const href = INTERNATIONAL_GRANTS_CARD_HREFS[index];
          const previewSrc = INTERNATIONAL_GRANTS_CARD_IMAGES[index];
