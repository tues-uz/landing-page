...
export default function InternationalCongressSustainabilityGreenTransformationPage() {
  ...
  const leafLabel = trConf(t, "internationalConferencesCard7BreadcrumbLabel");
...
function ConferenceArticle({ t }: { t: TFunction }) {
  const title = trConf(t, "internationalConferencesCard7PageTitle");
  const alt = trConf(t, "internationalConferencesCard7HeroAlt");
  const bodyRaw = trConf(t, "internationalConferencesCard7DetailBody");
  ...
  return (
    <article className="max-w-none">
      <figure className="relative aspect-[16/10] w-full overflow-hidden rounded-xl border border-border bg-muted shadow-sm sm:aspect-[16/9]">
        <img
          src={INTERNATIONAL_CONFERENCE_CARD7_HERO_SRC}
          alt={alt}
