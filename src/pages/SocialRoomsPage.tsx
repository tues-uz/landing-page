...
import { SOCIAL_ROOMS_DEFAULTS } from "@/locales/socialRoomsDefaults";

function trSocialRooms(t: TFunction, key: keyof typeof SOCIAL_ROOMS_DEFAULTS) {
  return t(key, { defaultValue: SOCIAL_ROOMS_DEFAULTS[key] });
}
...
export default function SocialRoomsPage() {
  ...
  const navLeafLabel = th("secondNavStudentLife.socialRooms");
...
function SocialRoomsArticle({ t }: { t: TFunction }) {
  const title = trSocialRooms(t, "socialRoomsPageTitle");
  const bodyRaw = trSocialRooms(t, "socialRoomsPageBody");
  const heroSrc = trSocialRooms(t, "socialRoomsHeroSrc");
