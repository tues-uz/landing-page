...
import { MUSHOIRA_CLUB_HONORED_DEFAULTS } from "@/locales/mushoiraClubHonoredDefaults";

function trMushoira(t: TFunction, key: keyof typeof MUSHOIRA_CLUB_HONORED_DEFAULTS) {
  return t(key, { defaultValue: MUSHOIRA_CLUB_HONORED_DEFAULTS[key] });
}
...
export default function MushoiraClubHonoredPage() {
  const { t } = useTranslation("topNav");
  const { t: tCommon } = useTranslation("common");
  const { t: th } = useTranslation("header");

  const leafTitle = trMushoira(t, "mushoiraClubHonoredPageTitle");
        ...
        <div className="container mx-auto max-w-[1348px] px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-12 lg:gap-8">
            <div className="order-1 lg:order-none lg:col-span-9">
              <MushoiraClubHonoredArticle t={t} />
            </div>
...
function MushoiraClubHonoredArticle({ t }: { t: TFunction }) {
  const title = trMushoira(t, "mushoiraClubHonoredPageTitle");
  const bodyRaw = trMushoira(t, "mushoiraClubHonoredPageBody");
  const heroSrc = trMushoira(t, "mushoiraClubHonoredHeroSrc");
