...
} from "@/locales/admission2025MenuDefaults";
import { cn } from "@/lib/utils";

function trMenu(t: TFunction, key: keyof typeof ADMISSION_2025_MENU_PAGE_DEFAULTS) {
  return t(key, { defaultValue: ADMISSION_2025_MENU_PAGE_DEFAULTS[key] });
}

export default function Admission2025MenuPage() {
  const { t } = useTranslation("topNav");
  const { t: tCommon } = useTranslation("common");
  const { t: th } = useTranslation("header");

  const admissionLabel = th("secondNav.admission2025");
  const title = trMenu(t, "admission2025MenuPageTitle");
...
function Admission2025MenuArticle({ t }: { t: TFunction }) {
  const title = trMenu(t, "admission2025MenuPageTitle");
  const intro = trMenu(t, "admission2025MenuPageIntro");
  ...
  return (
    <article className="max-w-none">
      ...
      <ul className="mt-10 grid list-none grid-cols-1 gap-[16px] p-0 md:grid-cols-3" role="list">
        {ADMISSION_2025_MENU_CARD_KEYS.map((card) => {
          const cardTitle = trMenu(t, card.titleKey);
          const href = ADMISSION_2025_MENU_CARD_HREFS[card.id];
          const previewSrc = ADMISSION_2025_MENU_CARD_IMAGES[card.id];
