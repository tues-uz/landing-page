...
import { QIZLARJON_CHARITY_EVENT_DEFAULTS } from "@/locales/qizlarjonCharityEventDefaults";

function trQizlarjon(t: TFunction, key: keyof typeof QIZLARJON_CHARITY_EVENT_DEFAULTS) {
  return t(key, { defaultValue: QIZLARJON_CHARITY_EVENT_DEFAULTS[key] });
}
...
export default function QizlarjonCharityEventPage() {
  const { t } = useTranslation("topNav");
  const { t: tCommon } = useTranslation("common");
  const { t: th } = useTranslation("header");

  const leafTitle = trQizlarjon(t, "qizlarjonCharityPageTitle");
...
function QizlarjonCharityArticle({ t }: { t: TFunction }) {
  const title = trQizlarjon(t, "qizlarjonCharityPageTitle");
  const bodyRaw = trQizlarjon(t, "qizlarjonCharityPageBody");
  const heroSrc = trQizlarjon(t, "qizlarjonCharityHeroSrc");
