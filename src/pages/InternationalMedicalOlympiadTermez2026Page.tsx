...
export default function InternationalMedicalOlympiadTermez2026Page() {
  ...
  const leafLabel = trConf(t, "internationalConferencesCard8BreadcrumbLabel");
...
function ConferenceArticle({ t }: { t: TFunction }) {
  const title = trConf(t, "internationalConferencesCard8PageTitle");
  const alt = trConf(t, "internationalConferencesCard8HeroAlt");
  const bodyRaw = trConf(t, "internationalConferencesCard8DetailBody");
  ...
  return (
    <article className="max-w-none">
      <figure className="relative aspect-[16/10] w-full overflow-hidden rounded-xl border border-border bg-muted shadow-sm sm:aspect-[16/9]">
        <img
          src={INTERNATIONAL_CONFERENCE_CARD8_HERO_SRC}
          alt={alt}
