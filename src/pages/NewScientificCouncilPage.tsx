...
import { NEW_SCIENTIFIC_COUNCIL_PAGE_DEFAULTS } from "@/locales/newScientificCouncilDefaults";

function trNewCouncil(t: TFunction, key: keyof typeof NEW_SCIENTIFIC_COUNCIL_PAGE_DEFAULTS) {
  return t(key, { defaultValue: NEW_SCIENTIFIC_COUNCIL_PAGE_DEFAULTS[key] });
}

export default function NewScientificCouncilPage() {
  ...
  const leafTitle = trNewCouncil(t, "newScientificCouncilPageTitle");
        ...
        <div className="container mx-auto max-w-[1348px] px-4 py-8 sm:px-6 lg:px-8 lg:py-10">
          <div className="grid grid-cols-1 gap-10 lg:grid-cols-12 lg:gap-8">
            <div className="order-1 lg:order-none lg:col-span-9">
              <NewScientificCouncilArticle t={t} />
            </div>
...
function NewScientificCouncilArticle({ t }: { t: TFunction }) {
  const title = trNewCouncil(t, "newScientificCouncilPageTitle");
  const bodyRaw = trNewCouncil(t, "newScientificCouncilPageBody");
  const paragraphs = bodyRaw
    .split(/\n\n+/)
    .map((p) => p.trim())
    .filter(Boolean);

  const heroSrc = "/images/about/new-scientific-council-hero.png";
