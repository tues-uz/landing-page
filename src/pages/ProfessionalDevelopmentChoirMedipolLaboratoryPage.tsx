...
export default function ProfessionalDevelopmentChoirMedipolLaboratoryPage() {
  ...
  const leafLabel = trChoir(t, "professionalDevelopmentChoirCard3BreadcrumbLabel");
...
function ChoirDetailArticle({ t }: { t: TFunction }) {
  const title = trChoir(t, "professionalDevelopmentChoirCard3PageTitle");
  const alt = trChoir(t, "professionalDevelopmentChoirCard3HeroAlt");
  const bodyRaw = trChoir(t, "professionalDevelopmentChoirCard3DetailBody");
  ...
  return (
    <article className="max-w-none">
      <figure className="relative aspect-[16/10] w-full overflow-hidden rounded-xl border border-border bg-muted shadow-sm sm:aspect-[16/9]">
        <img
          src={PROFESSIONAL_DEVELOPMENT_CHOIR_CARD3_HERO_SRC}
          alt={alt}
