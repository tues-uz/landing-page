...
export default function ProfessionalDevelopmentChoirIndonesiaMedicalInternshipPage() {
  ...
  const leafLabel = trChoir(t, "professionalDevelopmentChoirCard8BreadcrumbLabel");
...
function ChoirDetailArticle({ t }: { t: TFunction }) {
  const title = trChoir(t, "professionalDevelopmentChoirCard8PageTitle");
  const alt = trChoir(t, "professionalDevelopmentChoirCard8HeroAlt");
  const bodyRaw = trChoir(t, "professionalDevelopmentChoirCard8DetailBody");
  ...
  return (
    <article className="max-w-none">
      <figure className="relative aspect-[16/10] w-full overflow-hidden rounded-xl border border-border bg-muted shadow-sm sm:aspect-[16/9]">
        <img
          src={PROFESSIONAL_DEVELOPMENT_CHOIR_CARD8_HERO_SRC}
          alt={alt}
