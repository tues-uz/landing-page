...
export default function ProfessionalDevelopmentChoirIndonesiaInternshipDeparturePage() {
  ...
  const leafLabel = trChoir(t, "professionalDevelopmentChoirCard6BreadcrumbLabel");
...
function ChoirDetailArticle({ t }: { t: TFunction }) {
  const title = trChoir(t, "professionalDevelopmentChoirCard6PageTitle");
  const alt = trChoir(t, "professionalDevelopmentChoirCard6HeroAlt");
  const bodyRaw = trChoir(t, "professionalDevelopmentChoirCard6DetailBody");
  ...
  return (
    <article className="max-w-none">
      <figure className="relative aspect-[16/10] w-full overflow-hidden rounded-xl border border-border bg-muted shadow-sm sm:aspect-[16/9]">
        <img
          src={PROFESSIONAL_DEVELOPMENT_CHOIR_CARD6_HERO_SRC}
          alt={alt}
