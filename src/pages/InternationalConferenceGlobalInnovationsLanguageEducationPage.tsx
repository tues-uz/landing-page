...
const HUB_PATH = "/internationalization/international-conferences";
...
export default function InternationalConferenceGlobalInnovationsLanguageEducationPage() {
  const { t } = useTranslation("topNav");
  const { t: tCommon } = useTranslation("common");
  const { t: th } = useTranslation("header");

  const internationalizationLabel = th("secondNav.internationalization");
  const hubTitle = trConf(t, "internationalConferencesPageTitle");
  const leafLabel = trConf(t, "internationalConferencesCard5BreadcrumbLabel");
...
function ConferenceArticle({ t }: { t: TFunction }) {
  const title = trConf(t, "internationalConferencesCard5PageTitle");
  const alt = trConf(t, "internationalConferencesCard5HeroAlt");
  const bodyRaw = trConf(t, "internationalConferencesCard5DetailBody");
  ...
  return (
    <article className="max-w-none">
      <figure className="relative aspect-[16/10] w-full overflow-hidden rounded-xl border border-border bg-muted shadow-sm sm:aspect-[16/9]">
        <img
          src={INTERNATIONAL_CONFERENCE_CARD5_HERO_SRC}
          alt={alt}
